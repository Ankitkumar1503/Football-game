...
const POSITION_PROFILES = [
  ...
  },
  {
    ...
    number: "2+3",
    physical: ["SPEED-ENDURANCE (AEROBIC AND ANAEROBIC)", "EXPLOSIVE SPEED"],
    technical: [
      "DEFENSIVE TECHNIQUE",
      "TACKLING +",
      "SLIDING TACKLES",
    ...
    mental: ["AGGRESSIVENESS", "WILLPOWER AND CONFIDENCE"],
  },
  {
  ...
  },
  {
    ...
    number: "6",
    physical: ["ENDURANCE (AEROBIC)", "STRENGTH (IN THE DUEL)", "MOBILITY"],
    technical: [
      "DEFENSIVE TECHNIQUE",
      "PASSING",
      "RECEIVING THE BALL AND SPECIFIC CONTROL DRIBBLING THE BALL AWAY FOR DISTRIBUTION UPFIELD",
    ],
    tactical: ["POSITIONING AND REPOSITIONING", "ANTICIPATION", "PRESSING"],
    mental: ["FIGHTING QUALITIES", "HUMILITY", "CO-OPERATION", "WILLPOWER"],
  },
  {
    position: "LEFT AND RIGHT-SIDE MIDFIELDER",
    number: "7+8",
    physical: ["ENDURANCE (AEROBIC AND ANAEROBIC)", "SPEED"],
    technical: ["RUNNING WITH THE BALL", "DRIBBLING", "CROSSING", "SHOOTING"],
    tactical: [
  ...
  },
  {
    ...
    mental: ['"SELFISHNESS"', "OPPORTUNISM", "TRICKERY", "PERSEVERANCE"],
  },
  {
    position: "THE STRATEGIST",
    number: "10",
    physical: ["DEPENDING ON THE TYPE OF PLAYER (AND ON THE PLAYING STYLE)"],
    technical: [
...
export function PlayerProfilesByPosition() {
  return (
    <Card className="mb-[12px] bg-white dark:bg-[#1A1A1A] border-none shadow-none">
                  ...
                  <td className="border-2 border-black dark:border-white/20 p-[4] align-top bg-white dark:bg-[#1A1A1A]">
                    <ul className="space-y-1">
                      {profile.physical.map((item, i) => (
                        <li
                          key={i}
                          className="text-[8px] md:text-[9px] font-bold uppercase text-gray-800 dark:text-gray-300 leading-tight"
                        >
                          • {item}
                        </li>
                      ))}
