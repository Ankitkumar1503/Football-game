...
export function ActionWheel() {
    ...
    return (
        <div className="w-full max-w-5xl mx-auto mb-12 select-none flex flex-col items-center">
            ...
            {/* Header Grid */}
            <div className="w-full flex justify-between items-start px-4 md:px-12 mb-6">
                <div className="flex flex-col items-center gap-2">
                    <div className="w-16 h-16 md:w-20 md:h-20 rounded-full border-[3px] border-[#FF4422] flex items-center justify-center bg-transparent shadow-[0_0_15px_rgba(255,68,34,0.4)]">
                        <Activity className="size-8 md:size-10 text-[#FF4422]" strokeWidth={2} />
                    </div>
                    <span className="text-xl md:text-2xl font-black uppercase text-white tracking-wider">TOUCH</span>
                </div>

                <div className="text-center mt-4 max-w-md">
                    <h2 className="text-gray-300 font-display font-medium uppercase tracking-wide text-xs md:text-sm leading-relaxed">
                        AFTER YOU SELECT A TOUCH - THEN PICK AN ACTION<br />
                        THEN DECIDE IF IT WAS A NEGATIVE OR A POSITIVE TOUCH
                    </h2>
                </div>

                <div className="flex flex-col items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity" onClick={resetSelection}>
                    <div className="w-16 h-16 md:w-20 md:h-20 rounded-full border-[3px] border-[#FF4422] flex items-center justify-center bg-transparent shadow-[0_0_15px_rgba(255,68,34,0.4)]">
                        <Undo2 className="size-8 md:size-10 text-[#FF4422]" strokeWidth={2} />
                    </div>
                    <span className="text-xl md:text-2xl font-black uppercase text-white tracking-wider">UNDO</span>
                </div>
            </div>
            ...
            {/* Red/Yellow Cards */}
            <div className="flex justify-between w-full max-w-md px-8 mt-4">
                <button
                    onClick={() => handleActionClick('Red Card')}
                    className="flex flex-col items-center gap-1 group"
                >
                    <div className="w-12 h-16 bg-[#FF4422] rounded-sm shadow-[0_0_15px_rgba(255,68,34,0.5)] group-hover:scale-110 transition-transform cursor-pointer"></div>
                ...
                <button
                    ...
                    className="flex flex-col items-center gap-1 group"
                >
                    <div className="w-12 h-16 bg-[#EAB308] rounded-sm shadow-[0_0_15px_rgba(234,179,8,0.5)] group-hover:scale-110 transition-transform cursor-pointer"></div>
